...
Create RDD As User(Need Admin Approval 1)
    Create Session    mysesion    ${STAGE_URL}    verify=true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    # Configura las opciones de la solicitud (headers, auth)
    ...
    ${rddIdNaruto}=    Set Variable    ${response.json()}[_id]
    Set Global Variable    ${rddIdNaruto}
 ...
 Approve RDD
    ...
    ...    mysesion
    ...    url=/api/v1/admin/pb/odd/approve/${rddIdNaruto}?community=${idComunidad}
    ...    data={}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
...
Verify If RDD Id Nico Equals RDD Pedro(Join)
    Should Be Equal As Strings    ${rddId}    ${rddIdNaruto}
...
Assign Driver
    Create Session    mysesion    ${STAGE_URL}    verify=true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ${jsonBody}=    Set Variable    {"driver":{"driverId":"6582e724938e4e7b3bcf9f0a","driverCode":"753"}}
    ${parsed_json}=    Evaluate    json.loads($jsonBody)    json
...
Assign Vehicle
    Create Session    mysesion    ${STAGE_URL}    verify=true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ${jsonBody}=    Set Variable    {"vehicleId":"${vehicleId}"}
    ${parsed_json}=    Evaluate    json.loads($jsonBody)    json
    # Configura las opciones de la solicitud (headers, auth)
    ${headers}=    Create Dictionary    Authorization=${tokenAdmin}    Content-Type=application/json; charset=utf-8
    # Realiza la solicitud GET en la sesión por defecto
...
Get User QR(Naruto)
    ...
    ${qrCodeNaruto}=    Set Variable    ${response.json()}[0][qrCode]
    Set Global Variable    ${qrCodeNaruto}
    Log    ${qrCodeNaruto}
    Log    ${code}

Validate With QR(Naruto)
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ...    mysesion
    ...    url=/api/v2/pb/driver/departures/validate
    ...    data={"validationString":"${qrCodeNaruto}"}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
