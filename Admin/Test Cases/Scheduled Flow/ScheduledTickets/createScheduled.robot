...
Get Service Id
    ...
    # Calculamos el indice del ultimo objeto
    ${last_scheduled_service_index}=    Evaluate    ${num_scheduled_services} - 1

    # {Almacenamos el ultimo objeto de scheduledServices}
    ${last_scheduled_service}=    Set Variable    ${responseJson['scheduledServices'][${last_scheduled_service_index}]}

    Log    El último objeto scheduledServices es: ${last_scheduled_service}

    # Obtenemos el id de la ruta asociada al servicio, esta debe ser igual al id de la ruta calendarizada
    ${routeId}=    Set Variable    ${last_scheduled_service}[routeId][_id]
    Should Be Equal As Strings    ${scheduleId}    ${routeId}
...
Get departureId
    ...
    # Obtenemos el id de la ruta asociada al servicio, esta debe ser igual al id de la ruta calendarizada
    ${departureId}=    Set Variable    ${last_scheduled_service}[resources][0][departure][departureId]
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    Should Be Equal As Numbers    ${response.status_code}    200
